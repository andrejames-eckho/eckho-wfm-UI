from fastapi import FastAPI, HTTPException, Depends, status
from fastapi.middleware.cors import CORSMiddleware
from fastapi.security import HTTPBearer, HTTPAuthorizationCredentials
from datetime import datetime, timedelta
from typing import List, Optional
import json

from models import (
    LoginRequest, LoginResponse, UserRole, EmployeeStatus,
    TimeTrackingRequest, TimeTrackingResponse, TimeTrackingData,
    EmployeeListResponse, TimeRecordsResponse, DateRangeRequest,
    StatusUpdateRequest, Employee, TimeRecord
)
from auth import (
    authenticate_admin, authenticate_employee, create_access_token,
    verify_token, ACCESS_TOKEN_EXPIRE_MINUTES
)
from data_store import (
    all_employees, current_day_tracking, initialize_employee_tracking,
    get_employee_by_id, generate_time_records, get_employees_with_time_records_for_date,
    get_employee_status, status_colors
)

app = FastAPI(title="Eckho WFM Backend", version="1.0.0")

# Configure CORS
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:5173", "http://127.0.0.1:3000", "http://127.0.0.1:5173"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

security = HTTPBearer()

def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Dependency to get current authenticated user"""
    token = credentials.credentials
    payload = verify_token(token)
    if payload is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid authentication credentials",
            headers={"WWW-Authenticate": "Bearer"},
        )
    return payload

@app.get("/")
async def root():
    """Root endpoint"""
    return {"message": "Eckho WFM Backend API", "version": "1.0.0"}

@app.post("/api/auth/login", response_model=LoginResponse)
async def login(request: LoginRequest):
    """Authenticate user login"""
    try:
        # Check admin credentials first
        if authenticate_admin(request.username, request.password):
            access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
            access_token = create_access_token(
                data={"sub": request.username, "role": UserRole.ADMIN},
                expires_delta=access_token_expires
            )
            return LoginResponse(
                success=True,
                message="Admin login successful",
                user_type=UserRole.ADMIN,
                token=access_token
            )
        
        # Check employee credentials
        employee_id = authenticate_employee(request.username, request.password)
        if employee_id:
            access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
            access_token = create_access_token(
                data={"sub": request.username, "role": UserRole.EMPLOYEE, "employee_id": employee_id},
                expires_delta=access_token_expires
            )
            return LoginResponse(
                success=True,
                message="Employee login successful",
                user_type=UserRole.EMPLOYEE,
                employee_id=employee_id,
                token=access_token
            )
        
        return LoginResponse(
            success=False,
            message="Invalid username or password"
        )
    
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Login error: {str(e)}")

@app.get("/api/employees", response_model=EmployeeListResponse)
async def get_employees(current_user: dict = Depends(get_current_user)):
    """Get all employees (admin only)"""
    if current_user.get("role") != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Admin access required")
    
    try:
        # Update employee statuses based on current time tracking
        updated_employees = []
        for emp in all_employees:
            tracking = current_day_tracking.get(emp.id)
            if tracking:
                updated_emp = Employee(
                    id=emp.id,
                    firstName=emp.firstName,
                    lastName=emp.lastName,
                    timeIn=tracking.timeIn or emp.timeIn,
                    timeOut=tracking.timeOut or emp.timeOut,
                    breakIn=tracking.breakIn or emp.breakIn,
                    breakOut=tracking.breakOut or emp.breakOut,
                    status=tracking.status,
                    expectedStartTime=getattr(emp, 'expectedStartTime', None)
                )
            else:
                updated_emp = emp
            updated_employees.append(updated_emp)
        
        return EmployeeListResponse(
            employees=updated_employees,
            total=len(updated_employees)
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching employees: {str(e)}")

@app.get("/api/employees/{employee_id}")
async def get_employee(employee_id: int, current_user: dict = Depends(get_current_user)):
    """Get specific employee details"""
    # Admin can view any employee, employees can only view themselves
    if (current_user.get("role") != UserRole.ADMIN and 
        current_user.get("employee_id") != employee_id):
        raise HTTPException(status_code=403, detail="Access denied")
    
    employee = get_employee_by_id(employee_id)
    if not employee:
        raise HTTPException(status_code=404, detail="Employee not found")
    
    # Get current tracking data
    tracking = current_day_tracking.get(employee_id)
    if tracking:
        employee.timeIn = tracking.timeIn or employee.timeIn
        employee.timeOut = tracking.timeOut or employee.timeOut
        employee.breakIn = tracking.breakIn or employee.breakIn
        employee.breakOut = tracking.breakOut or employee.breakOut
        employee.status = tracking.status
    
    return employee

@app.post("/api/time-tracking", response_model=TimeTrackingResponse)
async def time_tracking_action(request: TimeTrackingRequest, current_user: dict = Depends(get_current_user)):
    """Handle time tracking actions (time in/out, break in/out)"""
    # Employees can only track their own time, admins can track any employee's time
    if (current_user.get("role") != UserRole.ADMIN and 
        current_user.get("employee_id") != request.employeeId):
        raise HTTPException(status_code=403, detail="Access denied")
    
    try:
        employee = get_employee_by_id(request.employeeId)
        if not employee:
            raise HTTPException(status_code=404, detail="Employee not found")
        
        # Initialize tracking for employee
        tracking = initialize_employee_tracking(request.employeeId)
        
        # Get current time
        now = datetime.now()
        current_time = now.strftime('%I:%M %p')
        
        # Handle different actions
        if request.action == "time_in":
            if tracking.timeIn:
                return TimeTrackingResponse(
                    success=False,
                    message="Already timed in for today"
                )
            tracking.timeIn = current_time
            tracking.status = EmployeeStatus.ON_DUTY
            message = f"Timed in at {current_time}"
            
        elif request.action == "time_out":
            if not tracking.timeIn:
                return TimeTrackingResponse(
                    success=False,
                    message="Must time in first"
                )
            if tracking.timeOut:
                return TimeTrackingResponse(
                    success=False,
                    message="Already timed out for today"
                )
            tracking.timeOut = current_time
            tracking.status = EmployeeStatus.COMPLETED
            message = f"Timed out at {current_time}"
            
        elif request.action == "break_in":
            if not tracking.timeIn:
                return TimeTrackingResponse(
                    success=False,
                    message="Must time in first"
                )
            if tracking.breakIn and not tracking.breakOut:
                return TimeTrackingResponse(
                    success=False,
                    message="Already on break"
                )
            tracking.breakIn = current_time
            tracking.status = EmployeeStatus.ON_BREAK
            message = f"Break started at {current_time}"
            
        elif request.action == "break_out":
            if not tracking.breakIn:
                return TimeTrackingResponse(
                    success=False,
                    message="Must start break first"
                )
            if tracking.breakOut:
                return TimeTrackingResponse(
                    success=False,
                    message="Break already ended"
                )
            tracking.breakOut = current_time
            tracking.status = EmployeeStatus.ON_DUTY
            message = f"Break ended at {current_time}"
            
        else:
            raise HTTPException(status_code=400, detail="Invalid action")
        
        return TimeTrackingResponse(
            success=True,
            message=message,
            data=tracking
        )
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Time tracking error: {str(e)}")

@app.get("/api/time-tracking/{employee_id}")
async def get_time_tracking(employee_id: int, current_user: dict = Depends(get_current_user)):
    """Get current time tracking status for an employee"""
    # Employees can only view their own data, admins can view any employee's data
    if (current_user.get("role") != UserRole.ADMIN and 
        current_user.get("employee_id") != employee_id):
        raise HTTPException(status_code=403, detail="Access denied")
    
    employee = get_employee_by_id(employee_id)
    if not employee:
        raise HTTPException(status_code=404, detail="Employee not found")
    
    tracking = initialize_employee_tracking(employee_id)
    return tracking

@app.get("/api/time-records/{employee_id}", response_model=TimeRecordsResponse)
async def get_time_records(
    employee_id: int, 
    months: int = 3,
    current_user: dict = Depends(get_current_user)
):
    """Get historical time records for an employee"""
    # Employees can only view their own records, admins can view any employee's records
    if (current_user.get("role") != UserRole.ADMIN and 
        current_user.get("employee_id") != employee_id):
        raise HTTPException(status_code=403, detail="Access denied")
    
    employee = get_employee_by_id(employee_id)
    if not employee:
        raise HTTPException(status_code=404, detail="Employee not found")
    
    try:
        records = generate_time_records(employee, months)
        return TimeRecordsResponse(
            records=records,
            total=len(records)
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching time records: {str(e)}")

@app.post("/api/employees/date-records", response_model=EmployeeListResponse)
async def get_employees_for_date(
    request: DateRangeRequest,
    current_user: dict = Depends(get_current_user)
):
    """Get all employees with their time records for a specific date (admin only)"""
    if current_user.get("role") != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Admin access required")
    
    try:
        target_date = datetime.fromisoformat(request.startDate.replace('Z', '+00:00'))
        employees_with_records = get_employees_with_time_records_for_date(target_date)
        
        return EmployeeListResponse(
            employees=employees_with_records,
            total=len(employees_with_records)
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching employee records: {str(e)}")

@app.put("/api/employees/{employee_id}/status")
async def update_employee_status(
    employee_id: int,
    request: StatusUpdateRequest,
    current_user: dict = Depends(get_current_user)
):
    """Update employee status (admin only)"""
    if current_user.get("role") != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Admin access required")
    
    employee = get_employee_by_id(employee_id)
    if not employee:
        raise HTTPException(status_code=404, detail="Employee not found")
    
    try:
        # Update the employee's status in current tracking
        tracking = initialize_employee_tracking(employee_id)
        tracking.status = request.status
        
        return {"success": True, "message": f"Status updated to {request.status}"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error updating status: {str(e)}")

@app.get("/api/status-colors")
async def get_status_colors():
    """Get status color mappings"""
    return status_colors

@app.get("/api/dashboard/summary")
async def get_dashboard_summary(current_user: dict = Depends(get_current_user)):
    """Get dashboard summary statistics (admin only)"""
    if current_user.get("role") != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Admin access required")
    
    try:
        total_employees = len(all_employees)
        
        # Count employees by status
        status_counts = {}
        for emp in all_employees:
            tracking = current_day_tracking.get(emp.id)
            current_status = tracking.status if tracking else emp.status
            status_counts[current_status] = status_counts.get(current_status, 0) + 1
        
        return {
            "total_employees": total_employees,
            "status_breakdown": status_counts,
            "active_employees": status_counts.get(EmployeeStatus.ON_DUTY, 0),
            "on_break": status_counts.get(EmployeeStatus.ON_BREAK, 0),
            "overtime": status_counts.get(EmployeeStatus.OVERTIME, 0),
            "late": status_counts.get(EmployeeStatus.LATE, 0)
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching dashboard summary: {str(e)}")

if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)
